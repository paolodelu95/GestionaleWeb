...
public partial class TipoPagamentoView : UserControl
{
    ...
    {
        if (DataContext is not TipoPagamentoViewModel vm) return;

        var selezione = new List<TipoPagamento>();
        foreach (var item in Griglia.SelectedItems)
            if (item is TipoPagamento t)
                selezione.Add(t);
