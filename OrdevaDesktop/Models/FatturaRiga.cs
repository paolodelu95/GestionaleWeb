...
/// <summary>
/// Riga di una fattura (tabella <c>fatture_righe</c>). Le colonne SQLite sono
/// snake_case: il repository usa alias espliciti verso queste proprietà
/// PascalCase. I valori REAL (quantità/prezzo/sconto/iva) diventano
/// <see cref="decimal"/> per evitare errori di arrotondamento sui totali; i flag
/// INTEGER 0/1 (<c>scarica_magazzino</c>) diventano bool.
/// </summary>
public sealed class FatturaRiga
{
    ...
    /// <summary>Nome prodotto risolto via JOIN (non mappato su una colonna di fatture_righe).</summary>
    ...
    /// <summary>Se la riga scarica il magazzino (default true, come nel backend).</summary>
    ...
    /// <summary>IVA della riga = imponibile × iva/100. 0 per le note.</summary>
    public decimal IvaImporto => IsNota ? 0m : Imponibile * Iva / 100m;
