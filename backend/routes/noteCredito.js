...
function saveRighe(ncId, righe) {
  const stmt = db.prepare(`INSERT INTO note_credito_righe (nota_credito_id, prodotto_id, descrizione, quantita, prezzo, iva, unita_misura)
    VALUES (?,?,?,?,?,?,?)`);
  for (const r of righe) stmt.run(ncId, r.prodottoId || null, r.descrizione, r.quantita, r.prezzo, r.iva, r.unitaMisura || '');
}
...
function toDto(r) {
  const totale = db.prepare(`SELECT COALESCE(SUM(quantita * prezzo * (1 + iva/100)), 0) as t FROM note_credito_righe WHERE nota_credito_id=?`).get(r.id)?.t || 0;
  const imponibile = db.prepare(`SELECT COALESCE(SUM(quantita * prezzo), 0) as t FROM note_credito_righe WHERE nota_credito_id=?`).get(r.id)?.t || 0;
