...
db.exec(`
  ...
  DELETE FROM note_credito_righe;
  DELETE FROM note_credito;
...
const insP = db.prepare(`INSERT INTO prodotti (nome, categoria, prezzo, quantita, soglia_minima, unita_misura, codice, iva)
  VALUES (?,?,?,?,?,?,?,?)`);
const prodIds = prodotti.map(p => insP.run(p.nome, p.categoria, p.prezzo, p.quantita, p.soglia, p.um, p.codice, p.iva).lastInsertRowid);
...
const insC = db.prepare(`INSERT INTO clienti (ragione_sociale, email, telefono, via, cap, citta, provincia, stato, codice_fiscale, p_iva)
  VALUES (?,?,?,?,?,?,?,?,?,?)`);
const cliIds = clienti.map(c => insC.run(c.rs, c.email, c.tel, c.via, c.cap, c.citta, c.prov, 'Italia', c.cf, c.piva).lastInsertRowid);
